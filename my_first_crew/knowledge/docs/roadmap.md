...
> 本节记录已确认的方向与待评估的方向。**规划不等于承诺**：不绑定具体版本号，不设完成时间，随实际需求动态调整。
> 原有「短期 / 中期 / 长期计划」保留作为历史脉络，新方向统一在本节登记。
...
| AI Agent | 按厂商/模型 Token 消耗看板 | 实时监控每家 API 的 Token 用量和成本。✅ v1.22 仪表盘 + v1.26 Flow 真实采集已落地（`crew:stats`/`flow:stats` → `crew_usage` 表 → `/api/crew/usage/*`）；后续可按需补充 Provider 维度成本拆分 |
| 贴纸系统 | 锚点定位修复 | 数据驱动锚点架构，解决位置漂移 |
| 贴纸系统 | 浮动渲染显示 | 贴纸在阅读页与编辑器的所见即所得 |
| 工程化 | TypeScript 迁移评估 | 评估迁移成本与收益，可在新模块先试点 |
| 工程化 | 模块索引维护 | 已建立 `module-index.md`，后续随代码更新同步维护 |
| 可观测性 | 执行历史记录 | 将 Agent 执行结果存入 SQLite，便于回溯 |
| 可观测性 | 调用链路追踪 | Agent 间的调用关系和耗时分析 |

### 探索性方向（待评估）

| 方向 | 说明 |
|------|------|
| CrewAI 升级到 1.16+ | 评估 Flows 功能是否值得升级，以及兼容性成本 |
| 前端框架引入 | 评估 Vue/React 替代原生 ESM 的可行性（目前 130 个 JS 文件） |
| 移动端原生应用 | 评估是否需要独立移动端 App，还是继续优化响应式 |
| 多语言支持 | 评估 i18n 的投入成本和实际需求 |

### 约束与原则

- **规划不等于承诺**：版本号是参考，不是合同。时间充裕就多做，时间紧张就调整。
- **优先级动态调整**：需求变化时，Roadmap 可以重新排序，不需要“坚持原计划”。
- **文档同步**：每次有新的方向确认，更新 Roadmap；每次有新功能落地，更新 `module-index.md`。
- **不追求“大而全”**：优先解决当前痛点（如贴纸位置、AI 循环），而不是铺开所有方向。
