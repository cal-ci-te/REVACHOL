...
| 注释规范化 — 批次 5（backend/，Node CJS） | ✅ 已完成 | 27 / 28（2 个异常文件跳过） |
...
**backend/ 模块头覆盖率：27 / 28（96%）— 唯一例外见 §8.4**
...
### 8.4 backend/ 的异常文件（批次 5 跳过，未改造）

批次 5 的 28 个 `.cjs` 中，有 1 个无法按注释规范处理，另有 2 个属无内容或非 `.cjs`：

| 文件 | 异常 | 处理 |
|------|------|------|
| `backend/check.cjs` | **UTF-16LE + BOM 编码**，Node `require` 抛 `SyntaxError`，ESLint 报 `Parsing error: Unexpected character`；全仓无任何引用 | 跳过。注释规范对「运行时根本加载不了的文件」无意义，转码属功能修复而非注释整理 |
| `backend/storage/migration/migrate.cjs` | 0 字节空文件 | 跳过。无内容可改；审查工具会报「缺少模块头」，属预期 |
| `backend/scripts/seed-admin.js` | `.js` 而非 `.cjs` | 已一并按同一规范改造（批次 5b），未因扩展名不同而漏掉 |

**另发现 3 个孤儿调试脚本**（`check.cjs`、`check-tables.cjs`、`test.cjs`），全仓均无引用。
`check-tables.cjs` 与 `test.cjs` 已按规范改造注释，但**其存在本身待专项清理决策**——
删除文件超出注释工程范围，故本轮不动，仅在此登记。
