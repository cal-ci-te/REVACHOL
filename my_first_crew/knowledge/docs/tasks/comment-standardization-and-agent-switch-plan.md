...
| 4c | `js/admin` | 28 | ✅ |
| 5 | `backend/`（Node CJS） | 28 | ✅（27 改造 + 1 跳过） |
| 6 | `my_first_crew/`（Python → `#`，不含 tests） | 17 | ⏸ |
| 7 | `config` / `tests` / `scripts` / JSONC | 64 可改 + 5 排除 | [>] 方案已定 |
...
| 子批 | 范围 | 文件 | 提交 |
|:--:|------|:--:|------|
| 5a | 排查脚本 + 工具校验 + 存储层 | 9 | `020223c` |
| 5b | 种子脚本 + 草稿清理 + 路由增强 + 本地适配器 | 4 | `5685f09` |
| 5c | 文章/草稿路由 + WebSocket | 3 | `1986b1f` |
| 5d | 迁移脚本 + 用量路由 + 上传 + RustFS 适配器 | 4 | `fd3a026` |
| 5e | 贴图路由 + 认证模块 | 2 | `52ab04e` |
| 5f | 数据访问层 + 服务入口 | 2 | `33c0111` |
| 5g | Agent 状态模块 | 1 | `6d2f051` |
| 5h | 图标包路由 + Crew 路由 | 2 | `03c1e75` |

**接续起点：`my_first_crew/`（批次 6，Python，17 文件）**
...
| `0d95fc1` | `docs(plan): 同步注释工程进度至批次 4c 完成并归档缺陷与工具` | 1 文件 +451 |
| `020223c` | `style(backend): 注释规范化为基础工具与存储层三层结构` | 9 文件 +49/-21 |
| `5685f09` | `style(backend): 注释规范化为草稿清理与适配层三层结构` | 4 文件 +44/-44 |
| `1986b1f` | `style(backend): 注释规范化为文章/草稿路由与 WebSocket 三层结构` | 3 文件 +31/-17 |
| `fd3a026` | `style(backend): 注释规范化为迁移脚本与存储/上传三层结构` | 4 文件 +56/-40 |
| `52ab04e` | `style(backend): 注释规范化为贴图路由与认证模块三层结构` | 2 文件 +74/-141 |
| `33c0111` | `style(backend): 注释规范化为数据访问层与服务入口三层结构` | 2 文件 +80/-43 |
| `6d2f051` | `style(backend): 注释规范化为 Agent 状态模块的行注释` | 1 文件 +6/-2 |
| `03c1e75` | `style(backend): 注释规范化为图标包与 Crew 路由三层结构` | 2 文件 +31/-32 |
| `7436185` | `chore(scripts): 审查工具容错不可解析文件并统计跳过数` | 1 文件 +32/-5 |
| `a215af3` | `style: 恢复 19 个文件原有的「无末尾换行」状态` | 19 文件 +19/-19 |
| `9f523bb` | `fix(backend): 修复 check.cjs 编码使其可被 Node 加载` | 1 文件（转码） |
| `323ae1a` | `style(backend): 为孤儿调试脚本标注「无引用」事实` | 2 文件 +5/-2 |
| `4e68c4b` | `docs(plan): 记录 check.cjs 转码与孤儿脚本标注的处置结果` | 1 文件 +50/-8 |
| `412bc8b` | `chore(scripts): 新增 Python 侧 AST 等价校验工具` | 1 文件 +91 |
| `4ec67c5` | `style(flows): 注释规范化为基础包与调试脚本三层结构` | 4 文件 +19/-7 |
| `3ccbaa5` | `style(ui): 注释规范化为三个小型面板三层结构` | 3 文件 +20/-4 |
| `23548c7` | `style(flows): 注释规范化为状态模型与断点续跑三层结构` | 3 文件 +62/-24 |
...
### 8.4 backend/ 的异常文件与孤儿脚本（已处置）

`backend/` 共 28 个 `.cjs` + 1 个 `.js`。**除 1 个 0 字节空文件外，全部已按规范改造**：

| 文件 | 异常 | 处置 |
|------|------|------|
| `backend/check.cjs` | **UTF-16LE + BOM 编码**，Node `require` 抛 `SyntaxError`、ESLint 报 `Parsing error`，即该脚本此前完全无法运行；全仓无引用 | **已转码为 UTF-8（`9f523bb`）**。逐 token 核对：转码前后各 115 tokens 且序列一致，代码零改动。`node --check` 由失败转为通过 |
| `backend/storage/migration/migrate.cjs` | 0 字节空文件 | **保持不动**。无内容可改；审查工具报「缺少模块头」属预期，这是 `27/28` 而非 `28/28` 的唯一原因 |
| `backend/scripts/seed-admin.js` | `.js` 而非 `.cjs` | 已按同一规范改造（批次 5b），未因扩展名不同而漏掉 |

**3 个孤儿调试脚本**（`check.cjs`、`check-tables.cjs`、`test.cjs`）全仓均无引用。
按决策**不删除**，改为在各自模块头显式写明孤儿身份（`323ae1a`），使后来者不会
误判为在用代码，也为将来清理留下明确依据。`test.cjs` 另澄清其不属测试套件
（套件在 `tests/`，由 Vitest 运行），避免被误当作测试文件。

> 批次 5 采用的「逐行替换注释」方式对含行尾空白的文件尤其重要：以行号定位替换注释行，
> 可保证代码字节（含行尾空白与末尾换行）零改动，使 diff 仅含注释行。

### 8.5 批次 7 侦察结论与分组方案

**方法论已提炼为独立手册**：`knowledge/docs/ai-collaboration/comment-standardization-playbook.md`
（含标准原文、三条底线、SOP、工具链、16 个实战坑、决策点清单、快速上手清单）。

#### 文件构成与注释可行性

| 分组 | 文件 | 行数 | 注释语法 |
|---|--:|--:|---|
| `tests/`（Vitest） | 32 `.js` | 5119 | `//` ✓ |
| `e2e-tests/`（Playwright） | 9 `.js` + 2 `.md` | 698 | `//` ✓ |
| `my_first_crew/tests/`（pytest） | 5 `.py` | 649 | `#` ✓ |
| `scripts/*.sh` | 2 | 318 | `#` ✓ |
| `scripts/comment-audit/`（自研工具） | 2 `.mjs` + 3 `.py` | 624 | `//` / `#` ✓ |
| 根级 JS 配置 | 5 | 467 | `//` ✓ |
| 根级 JSON 配置 | 4 | ~130 | **✗ 语法不支持** |
| 编排与镜像（compose / Dockerfile / .env.example） | 5 | 287 | `#` ✓ |
| JSONC（`crew.jsonc` + `agents/*.jsonc`） | 6 | 474 | `//` ✓ |
| `package-lock.json` | 1 | 8418 | 生成物，排除 |

#### 影响方案的 4 项发现

| # | 发现 | 处置 |
|:--:|---|---|
| 1 | **JSON 硬约束**：`package.json` / `jsconfig.json` / `tsconfig.json` / `.prettierrc` 在语法上不能有注释 | 从范围**排除**（不是暂缓，是不可做） |
| 2 | **4 个文件带 UTF-8 BOM**：`.eslintrc.js` / `vite.config.js` / `tsconfig.json` / `.prettierrc` | 读写用 `utf-8-sig`，避免抹掉 BOM 造成非注释改动 |
| 3 | **`.eslintrc.js` 是死配置**：ESLint 10 用 flat config，该文件不会被加载；而 `eslint.config.js` 第 22 行称「兼容旧的 .eslintrc.js」，实际只是手工抄了规则（两处规则完全重复） | 按 §8.4 先例**标注死配置身份**并修正失真注释，不删除 |
| 4 | **自研工具自身不合规**：`audit.mjs` 缺 `// ！` 模块头 + 4 处分隔线；`codecmp.mjs` 缺模块头 | 随 7d 一并规范化（dogfooding） |

#### 分组方案（已确认）

| 子批 | 范围 | 文件 | 力度 |
|:--:|---|--:|---|
| 7a | 根级 JS 配置 | 5 | 完整三层 + BOM 处置 + 修正失真注释 |
| 7b | 编排与镜像 | 5 | 完整三层（`#`） |
| 7c | JSONC 配置 | 6 | 完整三层（`//`） |
| 7d | `scripts/` | 7 | 完整三层 + dogfooding 修正 |
| 7e | `tests/`（Vitest） | 32 | **轻量**：减 + 模块头 |
| 7f | `e2e-tests/`（Playwright） | 9 | **轻量**：减 + 模块头 |
| 7g | `my_first_crew/tests/`（pytest） | 5 | **轻量**：减 + 模块头 |

**测试文件采用轻量力度的理由**：标准要求「若没有这条注释，读者会不会卡住或改错？不会就不写」。
测试的断言本身即文档，套用完整三层会产生大量复述断言的噪声。故测试只做减法
（清分隔线/装饰性注释、行尾注释转前置）+ 每文件补 1 行模块头（说明测什么与前置依赖），
不新增函数级说明。

#### 登记待办（超出注释范畴）

- `tests/` 有 **4 组同名测试文件**分处不同目录，且 `vitest.config.js` 的
  `include: ["tests/**/*.{test,spec}.js"]` 会同时收集两者：
...
批次 5（28）已拆为 8 个子批、批次 6（17）已拆为 8 个子批，均已完成；
批次 7（64 可改 + 5 排除）已定 7 个子批方案，见 §8.5。
...
| 2026-09-13 | 续做：**批次 5（backend/）完成 27/28**。拆 8 个子批提交（`020223c`…`03c1e75`）。27 文件 token 级全等价，注释违规 0（仅 0 字节文件报缺模块头，属预期）；Vitest 436 passed；ESLint 规则级对照零新增零消除（232 条一致）。清除 19 处分隔线与 [MODIFIED]/[MONITOR]/[DEPLOY]/[FUTURE] 过程标记，改写 12 处行尾注释，JSDoc 与 /* */ 全部改为 //；修正 2 处注释与实现/现状不符（seed-admin 的「明文存储」、「routes/auth.cjs」失效引用）。跳过 `check.cjs`（UTF-16 无法加载）与 0 字节的 `migration/migrate.cjs`，详见 §8.4 |
| 2026-09-13 | 工具：`audit.mjs` 容错不可解析文件（`7436185`）——批次 5 全量审查时 `check.cjs` 使 acorn 抛错、整批检查中断，改为「跳过并计数」，单个坏文件不再掩盖其余结论 |
| 2026-09-13 | 修正：恢复 19 个文件原有的「无末尾换行」状态（`a215af3`）。这 19 个文件在批次 4a~4c 改写时被补上了末尾换行（当时尚未内置末尾换行校正），虽不影响 token 等价但超出「仅改注释」约定，逐文件核对确认差异仅末行换行标志 |
| 2026-09-13 | 修复：`check.cjs` 由 UTF-16LE+BOM 转码为 UTF-8（`9f523bb`）——该脚本此前 Node 无法加载、ESLint 报解析错误，全仓无引用。转码前后各 115 tokens 且序列一致，代码零改动；`node --check` 转为通过。此举使 backend/ 的**全部 27 个可改造 .cjs 均已完成**（余 1 个为 0 字节空文件） |
| 2026-09-13 | 决策落地：3 个孤儿调试脚本（`check.cjs`/`check-tables.cjs`/`test.cjs`）**不删除**，改为在模块头写明孤儿身份（`323ae1a`），使后来者不会误判为在用代码 |
| 2026-09-13 | 批次 6 关键决策（方案 A）：Python 的 docstring 是字符串字面量、可经 `__doc__` 读取，属代码而非注释，故**全部保留不动**，三层结构改用 `#` 行注释实现（`# ！` 模块头置于 docstring 之前）。全批次以「保留 AST 等价 + 保留 `__doc__` 可用性」为约束 |
...
| 2026-09-13 | 修正：删除本计划文档中一段被重复写入两次的引用块（§8.4 末尾的「逐行替换注释」说明），为先前编辑时的疏漏 |
| 2026-09-13 | 批次 7a 完成（`2843451`，5 个 JS 配置）：补模块头；2 个 BOM 文件以二进制方式保留 BOM；修 2 处失真注释（`.eslintrc.js` 死配置、`eslint.config.js` 误称「兼容旧配置」）；删版本标记「v1.12.2」。**实测确认一处配置陷阱**：`vitest.config.js` 的 `...viteConfig` 展开的是函数（`defineConfig(fn)` 原样返回，实测 `out === fn`），展开得空对象，故未继承 alias/plugins，`vite.config.js` 的 test 段亦不被使用 |
