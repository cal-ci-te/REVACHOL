...
**图标包（Icon Pack）功能 + 图标包文档阅读模式 + 向后兼容修复（WIP）**

> ⚠️ WIP：贴纸浮动渲染显示功能尚未修复，仍为 WIP 版本。

**图标包功能（新）：**
- 新增 33 键图标键名注册表（`js/services/icon-pack-keys.js`）：基础 UI 键（站点/目录/工具栏/箭头/标签/主题/搜索/可见性/贴纸库）+ 超现实箱子 10 键
- 新增前端处理器（`icon-pack-processor.js`）：zip 遍历（支持子目录）、PNG magic / SVG 安全扫描、尺寸检测（64–512px 推荐）、PNG 等比压缩（≤512px）、缺失/未知键警告
- 新增图标包服务（`icon-pack-service.js`）：状态缓存、上传/删除/改主题、按槽位应用到全站图标、主题切换自动重应用
- 后端新增 `/api/icon-packs*` 路由：上传（安全性校验）、列表、状态、图标二进制、主题绑定修改、删除；`icon_packs` / `icon_pack_icons` 数据表；存储适配器支持图标包独立目录/前缀
- 管理面板新增「📦 图标包管理」区：名称输入、主题多选/全选、zip 上传、包列表主题勾选与删除
- 工具栏新增 🎨 键名文档（三主题 tab + 灰/绿状态），改为与帮助一致的详情标签页阅读模式
- 新增依赖 `jszip@^3.10.1`

**向后兼容修复：**
- `CustomIconManager` 新增外部覆盖（`setExternalIcon`），`SiteIcon` 与 DirectoryIcon/UIIcon/BoxState 统一 external 优先级：主题包图标 > 旧单槽位 localStorage > 默认
- 修复无包无旧图标时站点默认图片被清成 fallback、包模式下旧图标上传/恢复覆盖包图标的问题

**测试：**
- 新增 33 个图标包单元测试（键表 / 处理器 / 服务），全量测试 374 个全部通过

### v1.23.0-wip

**自定义图标体系 + 管理员面板“自定义贴图”整合 + 贴图库样式重构（WIP）**

> ⚠️ WIP：贴纸浮动渲染显示功能尚未修复，仍为 WIP 版本。

**自定义图标体系：**
- 新增 `DirectoryIcon` 单例：侧边栏目录文件夹图标支持自定义图片（localStorage 持久化，折叠/展开保持图标）
- 新增 `UIIcon` 单例：顶部工具栏收起/展开图标、管理员控制台折叠箭头均支持自定义图片
- 管理员面板新增“🎨 自定义贴图”折叠整合区：站点图标、目录图标、工具栏图标、控制台箭头、纹理、箱盖/箱体、物品贴图统一管理
- 头像上传恢复管理员面板顶部原位置

**贴图库样式重构：**
- `deco-ui.js` 移除 HTML 模板内联样式，全部迁移至 `css/components/deco-ui.css`
- 🔄 切换样式按钮加宽至 80px（唯一视觉变化）
- 动态预览图改用 `data-preview` + 渲染后应用，保持视觉一致

**文案统一管理：**
- 管理员面板全部新增/原有硬编码文案收敛至 `js/utils/ui-strings.js` 的 `UI.admin`
